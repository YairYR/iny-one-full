...
export const POST = withErrorHandling(async (request: NextRequest, ctx: RouteContext<"/api/shorten">) => {
  ...
  const normalizedUrlCheck = z.url({
    protocol: /^https?$/,
    hostname: z.regexes.domain,
  }).safeParse(urlWithSuffix);

  if (!normalizedUrlCheck.success) {
    throw new ValidationError("Invalid url provided");
  }

  const normalizedUrl = normalizedUrlCheck.data;
  const urlInfo = parseUrl(normalizedUrl);

  if (
    urlInfo.domain === null ||
    urlInfo.isIp ||
    ["iny.one", "localhost"].includes(urlInfo.domain)
  ) {
    console.log("❌ La URL ingresada no es válida:", normalizedUrl);
    throw new ValidationError("Invalid url provided");
  }
  ...
  if (bannedDomains.has(urlInfo.domain)) {
    ...
    if (urlBanned.data !== null && urlBanned.data === false) {
      console.log("❗ El dominio de la URL ingresada está baneada:", normalizedUrl);
      throw new ValidationError("Error when validating url");
    }
  }

  const { destination, utm: utmParams } = buildDestination(normalizedUrl, utm);
  ...
  const { error } = await shorterRepo.create(
    user_id,
    slug,
    destination,
    utmParams,
